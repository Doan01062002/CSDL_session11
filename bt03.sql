...
-- 2,
DELIMITER //
create procedure GetCustomerByPhone(in pro_PhoneNumber varchar(15))
begin
select CustomerID, FullName, DateOfBirth, Address, Email from customers where PhoneNumber = pro_PhoneNumber;
end//
DELIMITER //

-- 3,
DELIMITER //
create procedure GetTotalBalance(in pro_CustomerID int, out pro_TotalBalance DECIMAL(15,2))
begin
select a.CustomerID,sum(a.Balance) as totalBalnce from accounts a
join customers c on c.CustomerID = a.CustomerID
where a.CustomerID = pro_CustomerID
...
-- 4,
DELIMITER //
create procedure IncreaseEmployeeSalary(in pro_EmployeeID int, out pro_Salary DECIMAL(10,2))
begin
update Employees set salary = salary*1.1
where pro_EmployeeID = EmployeeID;

select salary into pro_Salary
from Employees
where EmployeeID = pro_EmployeeID;
end//
DELIMITER //
...
call GetTotalBalance(5,@pro_TotalBalance);

call IncreaseEmployeeSalary(2,@pro_Salary);
select @pro_Salary;
